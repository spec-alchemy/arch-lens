...
2. 要求基线 current、design approval 为 current、`.puml`/SVG 候选已经 `apply-model`、modelCommit 非空且为 HEAD 祖先；否则停止代码修改并返回 review model。
...
7. 运行项目测试和契约验证，把实现代码提交到 Git；该 commit 由 verification.md 的 `implementation-commit` 引用，其内容标识由 `implementation-patch-id` 记录。
