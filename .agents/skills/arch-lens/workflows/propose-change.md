...
4. 确认当前 worktree 没有活动 Change Pack 且工作区干净，再在有效 HEAD 上运行 `arch-lens change new <id>`。并行变更必须建立独立 branch/worktree。
...
12. 运行 `arch-lens change render <id>`，为每个 add/modify 候选原子生成受版本控制的标准 SVG；delete 不保留 `.puml` 或 SVG。
13. 把 tasks 写成引用 AC 的实施步骤；它可以在批准后细化。
14. 运行 `arch-lens change status <id> --json` 和 `validate`，修复结构、路径、基线、SVG 或 PlantUML 事实错误。若基线 stale，先同步 Git，再显式 `change refresh-base` 并返回语义审查。
