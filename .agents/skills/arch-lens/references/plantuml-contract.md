...
- 标准 SVG 是可再生成、受版本控制的审查视图。每个 `.puml` 必须在相邻层级的 `rendered/` 中具有同路径 `.svg`；只能由 CLI 使用锁定受管 PlantUML 生成，不得手工维护。PlantUML 仍是唯一业务模型。
- 文件按业务能力或用例组织，例如 `scheduling/submit-schedule.sequence.puml`，不用 `frontend/`、`backend/` 作为默认分类。
- add/modify 候选的 Change Pack `.puml`/SVG 相对路径必须与 canonical 路径一致；delete 不保留候选文件。每个 worktree 最多一个活动变更。
...
- `init` 校验 Java 21+，并把固定版本和摘要的官方 PlantUML JAR 安装到用户缓存。标准镜像只使用有效受管 JAR；非门禁语法检查或自定义导出才可按显式 `ARCH_LENS_PLANTUML`、受管 JAR、PATH PlantUML 的顺序解析。
